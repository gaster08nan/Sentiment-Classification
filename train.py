...
def train():
    ...
    process_x, label, token = clfs.preprocessing(reviews, labels, init_max_len= 500, init_min_len = 50)
    x_train, x_test, y_train, y_test = ut.train_test_split(process_x, label, test_size = 0.4, random_state = 42)
    ...
    #train model
    history = clfs.train(x_train, y_train, epochs = config.epochs, batch_size = config.batch_size)
    #save weight
