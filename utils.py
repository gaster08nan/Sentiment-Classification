...
def cleaning(raw_data):
    ...
    for review in raw_data:
        ...
        #Remove punctuation
        new_review= "".join([char for char in review if char not in string.punctuation])
...
def get_review(soup_page, reviews = [], sentiments = []):
    ...
    for i,review in enumerate(html_page):
        ...
        try:
            user_review = review.find('div', attrs = {'class': 'text show-more__control'}).text.strip()
            tmp_span_list = review.find('span', attrs = {'class': 'rating-other-user-rating'}
                                ).findAll('span')
        except Exception as e:
            print("error: ",e)
            continue
        rating_score = int(tmp_span_list[0].text)/ int(tmp_span_list[-1].text.replace('/',''))
        if rating_score >= 0.5:
            sentiments.append(1)
        else:
            sentiments.append(0)
