...
class App(tk.Tk):
    def __init__(self):
        ...
        def predict():
            #get sequences
            seuqnce = review_txt.get('1.0','end')
            #Init model
            ...
            if predict_result[0] <= 0.5:
                result_lb.config(text='Possitive',  background = 'green', foreground = '#ffffff')
            else:
                result_lb.config(text='Negative', background = 'red', foreground = '#ffffff')
        ...
        #change frame button
        review_change_frame_btn = tk.Button(review_frame, text='Change to crawl Data Frame', command = change_to_crawl)
        ...
        # result frame - button
        show_result_button = ttk.Button(result_frame, text = 'Classiffy', command = classiffy_func)
        show_result_button.grid(column=0, row=1, padx=10, pady=10)

        #crawl data frame
        crawl_frame = tk.LabelFrame(self)
        #label
        input_url_lbl = tk.Label(crawl_frame, text = 'Input url to crawl reviews')
        ...
        #change frame button
        crawl_change_frame_btn = tk.Button(crawl_frame, text='Change to review Frame', command = change_to_main)
        crawl_change_frame_btn.grid(column = 0, row= 2, padx=10, pady=10)

        def crawl_review():
            url = input_url_txt.get()
            np_load = int(load_page_txt.get())
            return_reviews, return_sentiments = ut.crawl_data_from_url(url, np_load)
        ...
        crawl_review_btn = tk.Button(crawl_frame, text='Crawl review', command = crawl_review)
