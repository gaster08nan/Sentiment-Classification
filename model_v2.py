...
class model():
    ...
    def build_model(self, input_dim=1000, output_dim=128, node=32):
        """Buid model

        Args:
            input_dim (int, optional): input dimension for embedding layers. Defaults to 1000.

        Returns:
            _type_: return class model structure
        """
        model = Sequential()
    ...
    def preprocessing(self, reviews, label, old_tokenizer=None, init_max_len=500, init_min_len=10, remove=True):
        """
        ...
        tokenized_seq, seq_len, tokenizer = ut.tokenize(
            clean_data, old_tokenizer)
        ...
        if remove:
            ...
            new_tokenized_seq = tokenized_seq
            new_seq_len = seq_len
            new_label = label
    ...
    def predict_preprocessing(self, reviews, tokeninzer, max_leng, min_leng, index_reviews=None):
        """
        ...
        if index_reviews is not None:
            rm_seq, new_seq_leng, _, new_index = ut.remove_seq(
        ...
        if index_reviews is not None:
            return np.array(processed_seq), np.array(new_index)
        else:
...
def init_LSTM_Classifier():
    class LSTM_cls(model):
        def __init__(self) -> None:
            super().__init__()
            self.model = None

        def build_model(self, input_dim=1000, output_dim=1024, node=64):
            """
            Build LSTM model
            """
            model = Sequential()
...
def init_GRU_Classifier():
    class GRU_cls(model):
        def __init__(self) -> None:
            super().__init__()
            self.model = None

        def build_model(self, input_dim=1000, output_dim=1024, node=128):
            """
            Build GRU model
            """
...
def init_BiLSTM_LSTM_Classifier():
    class BiLSTM_cls(model):
        ...
        def build_model(self, input_dim=1000, output_dim=128, node=32):
            ...
            model.add(Embedding(input_dim=input_dim, output_dim=output_dim))
            model.add(
                Bidirectional(LSTM(node, return_sequences=True))
            )
